...
export class Grid {
  ...
  setup() {
    ...
    this.ctx = this.canvas.getContext('2d'); // Corrected here
  ...
  createBlocks() {
    ...
    for (let row = 0; row < rowCount; row++) {
      for (let col = 0; col < blockCount; col++) {
        ...
        const yPos = Math.floor(row * (this.blockWidth + this.blockGap)); // Corrected here
  ...
  setupEvents() {
    this.mouseMoveHandler = (e) => { // Corrected here
    ...
    this.canvas.addEventListener('mousemove', this.mouseMoveHandler); // Corrected here
